...
def newton_y(A, gamma, x, D, i):
    ...
    for j in range(255):
        ...
        y = (yfprime + 10**18 * D - 10**18 * S) // fprime + mul1 // fprime * (
            10**18 - K0
        ) // K0
...
def get_all():
    ...
    for name, pair in [("btcusdt", (0, 1)), ("ethusdt", (0, 2)), ("ethbtc", (1, 2))]:
...
class Trader:
        ...
        if t > self.t:
            ...
            for k in range(1, N):
                self.price_oracle[k] = int(
                    price_vector[k] * (1 - alpha) + self.price_oracle[k] * alpha
                )
            self.t = t
    ...
    def tweak_price(self, t, a, b, p):
        ...
        p_new = [10**18]
        p_new += [
            p_target + self.adjustment_step * (p_real - p_target) // norm
                ...
                try:
                    print(
                        (
                            ...
                            lasts.get(
                                (0, 1),
                                self.price_oracle[1] * 10**18 // self.price_oracle[0],
                            )
                            / 1e18,
                            lasts.get(
                                (0, 2),
                                self.price_oracle[2] * 10**18 // self.price_oracle[0],
                            )
                            / 1e18,
                            self.curve.p[1] / 1e18,
                            self.curve.p[2] / 1e18,
